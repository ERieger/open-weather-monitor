...
function mqttPatternExec(pattern, topic) {
	return matches(pattern, topic) ? extract(pattern, topic) : null;
}

function mqttPatternMatches(pattern, topic) {
...
function mqttPatternFill(pattern, params){
	var patternSegments = pattern.split(SEPARATOR);
	var patternLength = patternSegments.length;
...
function mqttPatternExtract(pattern, topic) {
	var params = {};
	var patternSegments = pattern.split(SEPARATOR);
	var topicSegments = topic.split(SEPARATOR);
...
function mqttPatternClean(pattern) {
	var patternSegments = pattern.split(SEPARATOR);
	var patternLength = patternSegments.length;
